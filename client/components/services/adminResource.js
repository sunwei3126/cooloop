angular.module('services.adminResource', []).factory('adminResource', ['$http', '$q', function ($http, $q) {
  ...
  var settingUrl = baseUrl + '/setting';
  ...
  resource.addUser = function(username){
    return $http.post(userUrl+ '/add', { username: username }).then(processResponse, processResponse);
  };
  ...
  resource.setUserPassword = function(_id, data){
  ...
  // ----- settings api -----
  resource.getMailbox = function(){
    var url = settingUrl + '/mailbox';
    return $http.get(url).then(processResponse, processError);
  };
  resource.setMailbox = function(setting){
    var url = settingUrl + '/mailbox';
    return $http.put(url, setting).then(processResponse, processError);
  };
